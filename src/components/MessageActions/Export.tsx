  ...
  const exportAsPDF = async () => {
    ...
    try {
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        doc.text(`Generated by Perplexify - Page ${i} of ${totalPages}`, margin, pageHeight - 10);
        doc.text(new Date().toLocaleDateString(), pageWidth - margin - 30, pageHeight - 10);
      }
      
      doc.save(`perplexify-message-${message.messageId}.pdf`);
    } catch (error) {
